...
// --- 子元件：出缺席 HUD (使用 React.memo 隔離每秒重繪) ---
const AttendanceHUD = React.memo(({ attendanceStats, onOpenAttendance, UI_THEME }) => {
  return (
    <div className="absolute top-8 right-8 z-30 pointer-events-auto hover:scale-110 transition-transform ease-out will-change-transform transform-gpu">
      <div className="flex flex-col items-center gap-2">
        ...
        {/* 未到名單提示 - 加入最高高度與自訂捲軸避免破版 */}
        {attendanceStats.absentees.length > 0 && (
          <div className={`mt-2 backdrop-blur text-sm rounded-xl p-3 max-w-[200px] max-h-[40vh] overflow-y-auto custom-scrollbar animate-in fade-in slide-in-from-top-2 shadow-sm hover:shadow-md ${UI_THEME.SURFACE_CARD}  ${UI_THEME.BORDER_DEFAULT}`}>
            <ul className="space-y-1">
              <span className="text-[20px] uppercase text-rose-400 font-bold sticky top-0 backdrop-blur-md">
                缺席
              </span>
              {attendanceStats.absentees.map(s => (
                <li key={s.id} className="flex flex-col border-b border-rose-500/10 last:border-0 pb-1 mb-1">
                  {s.isManualNote ? (
                    // 手動模式：顯示大段備註文字
                    <div className="flex flex-col gap-1 w-full">
                      <span className={`font-bold text-lg leading-relaxed whitespace-pre-wrap break-words block w-[150px] ${UI_THEME.TEXT_PRIMARY}`}>
                        {s.name}
                      </span>
                    </div>
                  ) : (
                    ...
                    <div className="flex flex-col gap-1 w-full">
                      <div className="flex justify-between items-center w-full">
                        <span className={`font-bold text-base ${UI_THEME.TEXT_PRIMARY}`}>{s.number} {s.name}</span>
                        <span className="text-[10px] px-1.5 py-0.5 bg-rose-100 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 rounded shrink-0 ml-1">{s.statusLabel}</span>
                      </div>
                    </div>
                  )}
                </li>
        ...
        <div className="w-full max-w-fulll px-4">
          <div className="flex flex-col items-center gap-10 animate-in slide-in-from-bottom-4 fade-in duration-700">
            ...
            {isExam && remainingSeconds > 0 && progress < 16 && (
              <div className="absolute inset-0 flex items-start justify-center pointer-events-none z-30 overflow-hidden transform-gpu will-change-transform will-change-opacity">
                <div className="absolute inset-0 bg-rose-500/10 dark:bg-rose-700/20 animate-pulse transform-gpu will-change-transform" />
                <p className="mt-10 text-rose-300/70 dark:text-rose-300 text-[10vw] font-black tracking-widest select-none drop-shadow-xl translate-z-0">
                  即將結束
                </p>
              </div>
            )}
